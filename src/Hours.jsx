...
function Hours({
  ...
  function handleUpdate() {
    ...
    if (esLibre == 1) {
      ...
      handlePopup(
        ...
        "No se puede guardar la informacion ya que la hora no se encuentra libre, si desea cambiar la informacion elimine la asesoria antes"
      );
    }
  }
  ...
  function updateData() {
    axios
      .put(API_URL + `api/asesores/actualizarDiaHora/`, {
        idDiaHora: idDiaHora,
      ...
      .then((response) => {
        ...
        if (response.data.error) {
          ...
          handlePopup(
            ...
            "Se actualizo la hora correctamente"
          );
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      {data ? (
        <>
          ...
          <FormControlLabel
            control={<Switch onChange={handleChangeSwitch} checked={status} />}
            label={labelSwitch}
          />
        </>
      ) : (
